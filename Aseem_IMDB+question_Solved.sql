...
(SELECT count(*) FROM movie WHERE country  is NULL) as year_null, 
...
FROM   movie
GROUP  BY year;
--  In Year 2017 highest no. of movies were released - 3052
...
-- Drama genre had the highest movies = 4285.
...
SELECT 
    genre_count, COUNT(movie_id) as movie_count
FROM
    (SELECT 
...
--  Duration of Action movies is highest = 112.88 mins whereas Horror movies have least with duration = 92.72 mins.
...
-- Thriller genre has 3rd rank with Movie count = 1484.
...
-- Dream Warrior Pictures and National Theatre Live production both have the most number of hit movies = 3 movies with average rating > 8
...
-- Drama genre had the maximum no. of releases with 24 movies 
...
-- The movies that are released between 1 April 2018 and 1 April 2019 and have given a median rating of 8 are 365 No.
...
WHERE
    upper(country) = 'germany'
        OR upper(country) = 'italy'
GROUP BY country;

-- Yes Germans gets more votes than italy
...
SELECT 
		SUM(CASE WHEN name IS NULL THEN 1 ELSE 0 END) AS col_name_nulls, 
...
SELECT genre,
       ...
       SUM(ROUND(AVG(duration),2)) OVER(ORDER BY genre ROWS UNBOUNDED PRECEDING) AS total_running_duration,
...
-- The Fate of the furious is top ranked in worldwide_gross_income
